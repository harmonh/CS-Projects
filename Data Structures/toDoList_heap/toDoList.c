/* CS261- Assignment 5 - compare.c*/
/* Name: Holly Harmon	
 * Date: 05/25/2014
 * Solution description: Functions for a to do list program using a heap
 * array interface to store items according to priority
 */
...
void printList(DynArr *heap)
{
	...
	copyDynArr(heap, newHeap);
	sortHeap(newHeap);

	printf("Your current tasks are:\n");
	for(int i = 0; i < sizeDynArr(newHeap); i++) {
		curTask = (Task*)getDynArr(newHeap, i);
		printf("%s\n", curTask->description);
	}
	/* FIXME: Write this */
}
...
{
	Task *tleft = (Task*)left;
	Task *tright = (Task*)right;
